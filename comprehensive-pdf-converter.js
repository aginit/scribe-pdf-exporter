      ...
      for (let i = 1; i <= pageCount; i++) {
        try {
          ...
          pageImages.push({
            page: i,
            filename: `page.${i}.png`,
            path: `${cleanName}_images/page.${i}.png`,
            width: 1280,
            height: 720
          });
          console.log(`  ✅ Page ${i} converted`);
        } catch (pageError) {
          console.log(`  ⚠️  Page ${i} failed: ${pageError.message}`);
        }
      }
    } catch (imageError) {
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node comprehensive-pdf-converter.js <pdf-file> [output-directory]');
    process.exit(1);
  }
  ...
  console.log('🚀 Starting comprehensive PDF conversion...');
