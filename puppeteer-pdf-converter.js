...
async function convertPdfToStructuredFormat(pdfPath, outputDir) {
  try {
    ...
    // Step 2: Extract actual images from PDF using Puppeteer
    console.log('🖼️  Extracting actual screenshots from PDF...');
    let pageImages = [];
    ...
    try {
      ...
      const filename = `pdf_screenshot.png`;
      ...
      // For now, use the single screenshot for all steps
      // In a more advanced version, we could try to detect page breaks
      for (let i = 1; i <= pdfData.numpages; i++) {
        const stepFilename = `step_${i}_page_${i}.png`;
        const stepPath = path.join(imagesDir, stepFilename);

        // Copy the main screenshot for each step
        await fs.copy(imagePath, stepPath);

        pageImages.push({
          page: i,
          filename: stepFilename,
          path: `${cleanName}_images/${stepFilename}`,
          width: 1280,
          height: 800,
      ...
      console.log(`⚠️  Screenshot extraction failed: ${imageError.message}`);
    ...
    console.log(`\n✅ Puppeteer conversion completed!`);
...
if (require.main === module) {
  ...
  if (args.length < 1) {
    console.log('Usage: node puppeteer-pdf-converter.js <pdf-file> [output-directory]');
    process.exit(1);
  }
