...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node high-quality-pdf-converter.js <pdf-file> [output-directory]');
    process.exit(1);
  }
  ...
  console.log('🚀 Starting high-quality PDF conversion...');
  const result = await convertPdfToStructuredFormat(pdfPath, outputDir);

  if (result.success) {
    console.log('\n✅ High-quality conversion completed!');
    ...
    console.log(`  Images: ${result.outputs.images}/ (placeholders ready)`);
  } else {
