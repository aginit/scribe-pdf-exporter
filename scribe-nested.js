...
async function downloadAllScribes() {
  ...
  console.log('🚀 Starting Complete Scribe Export with Subfolder Support...');
  ...
  try {
    ...
    // Main folders - these are the actual folder names in Scribe
    const mainFolders = [
    ...
    for (let i = 0; i < mainFolders.length; i++) {
      ...
      try {
        ...
        // Check for subfolders by looking for expandable elements
        // Look for arrows or expand icons that indicate subfolders
        const hasSubfolders = await page.$$eval(
          `[aria-label*="${folderName}"] svg, [aria-label*="${folderName}"] [class*="arrow"], [aria-label*="${folderName}"] [class*="expand"]`,
        ...
        if (hasSubfolders) {
          ...
          // Try to expand the folder
          try {
            // Click on expand arrow if it exists
            await page.click(`[aria-label*="${folderName}"] svg`, { timeout: 2000 });
            await page.waitForTimeout(1500);
          } catch (e) {
            // Folder might already be expanded or no subfolders
          }
          
          // Look for nested items under this folder
          // These would typically be indented or nested elements
          const subfolderElements = await page.$$eval(
            `[aria-label*="${folderName}"] + *, [aria-label*="${folderName}"] ~ [style*="padding-left"], [class*="nested"] a, [class*="child"] a`,
            elements => {
              return elements
          ...
          if (subfolderElements.length > 0) {
            console.log(`   📁 Found potential subfolders: ${subfolderElements.join(', ')}`);
            
            // Process each subfolder
            for (const subfolder of subfolderElements) {
              try {
                console.log(`   📂 Checking subfolder: ${subfolder}`);
                await page.click(`text=${subfolder}`, { timeout: 3000 });
                await page.waitForTimeout(2000);
                ...
                if (subfolderDocs.length > 0) {
                  // Only add if these are different documents
                ...
                console.log(`      ⚠️  Could not access subfolder ${subfolder}`);
              }
            }
          }
        }
    ...
    // Also check for documents at the root "All Documents" level
    console.log(`\n📂 Checking root level (All Documents)...`);
    await page.click('text=All Documents', { timeout: 3000 });
    await page.waitForTimeout(2000);
    ...
    if (rootDocs.length > 0) {
      // Only add documents not already found in folders
      const newRootDocs = rootDocs.filter(doc => 
        !allDocuments.some(existing => existing.href === doc.href)
      );
      if (newRootDocs.length > 0) {
        allDocuments.push(...newRootDocs);
        console.log(`   📄 Found ${newRootDocs.length} documents at root level`);
      }
    }
